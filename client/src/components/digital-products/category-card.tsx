...
export function CategoryCard({ category }: { category: DigitalProductCategory }) {
  return (
    <Link
    ...
    >
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        <span className="label absolute left-3 top-3 bg-paper/90 px-2 py-1 text-[10px]">{category.index}</span>
      </div>

      <div className="flex flex-1 flex-col p-3.5 sm:p-5">
        <div className="flex items-start justify-between gap-2">
          <h2 className="text-sm font-semibold tracking-tight sm:text-lg">{category.title}</h2>
          <ArrowUpRight
            size={16}
        ...
        <p className="mt-1.5 line-clamp-2 text-xs leading-relaxed text-graphite sm:text-sm">{category.summary}</p>
        <span className="label mt-auto pt-4 text-[10px] text-ink">Explore →</span>
      </div>
    </Link>
  );
}
