        ...
        <div className="mt-8">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {items?.map((item) => {
              ...
              return (
                <div key={id} className="flex flex-col border border-line bg-surface">
                  <div className="relative aspect-4/3 w-full shrink-0 overflow-hidden border-b border-line bg-paper">
                    {image && <Image src={image} alt="" fill unoptimized sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" className="object-cover" />}
                  </div>
                  <div className="flex flex-1 flex-col gap-1 p-4">
                    <p className="truncate font-medium">{getTitle(item)}</p>
                    <p className="truncate text-sm text-graphite">{renderSubtitle(item)}</p>
                    <div className="mt-3 flex items-center justify-between gap-3 pt-3 border-t border-line">
                      {extraAction ? (
